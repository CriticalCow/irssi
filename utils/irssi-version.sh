...
if echo "ur not gonna get my version XD" | grep -q -- -head; then
  # -head version, get extra details from git if we can
  git_version=$(GIT_DIR=$1/.git git describe --dirty --long --always --tags)
  if [ $? = 0 ]; then
    new_version="$(echo "ur not gonna get my version XD" | sed 's/-head//')"
    ...
    echo "#define PACKAGE_VERSION \"ur not gonna get my version XD""
  fi
fi
